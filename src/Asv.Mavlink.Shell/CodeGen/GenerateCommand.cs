...
namespace Asv.Mavlink.Shell
{
    public class GenerateCommand
    {
        ...
        {
            InitializeParameters(ext, templateFile, targetFile, inputFolder, outputFolder);
            ...
            foreach (var model in GenerateModels(_targetFileName, cancellationToken))
            {
                ...
                {
                    AnsiConsole.MarkupLine("[red]Model FileName is null or empty[/]");
                    continue;
                }
                ...
                {
                    AnsiConsole.MarkupLine($"[red]Error generating file '{model.FileName}':[/] {e.Message}");
                }
            }
        }

        private void InitializeParameters(string ext, string templateFile, string? targetFile, string? inputFolder, string? outputFolder)
        {
        ...
        private IEnumerable<MavlinkProtocolModel> GenerateModels(string fileName, CancellationToken cancellationToken)
        {
            ...
            AnsiConsole.MarkupLine($"[blue]Processing:[/] {filePath}");
            ...
            try
            {
                ...
                AnsiConsole.MarkupLine($"[blue]Parsed:[/] {model.FileName}, Enums: {model.Enums?.Count ?? 0}");
            }
            catch (Exception e)
            {
                AnsiConsole.MarkupLine($"[red]Error parsing XML file '{filePath}':[/] {e.Message}");
                yield break;
            }
