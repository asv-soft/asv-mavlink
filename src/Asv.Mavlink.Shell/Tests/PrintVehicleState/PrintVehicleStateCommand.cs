...
public class PrintVehicleStateCommand : IDisposable
{
    ...
    private VehicleStateRenderer _vehicleRenderer;
    ...
    public async Task<int> Run(string connection, uint refreshRate = 3000)
    {
        try
        {
            ...
            _vehicleRenderer = new VehicleStateRenderer(_device);
            _logRenderer = new LogRenderer();

            var ui = new UiRenderer(_vehicleRenderer, _logRenderer);
            var uiTask = ui.RunAsync(_cancel.Token);

            using var sub = _keyListener.KeyPressed.SubscribeAwait(async (action, _) => await KeyPressedAsync(action, _cancel.Token));
    ...
    private async ValueTask KeyPressedAsync(VehicleAction action, CancellationToken cancel)
    {
        ...
        switch (action)
        {
            ...
            case VehicleAction.PageUp:
                ...
                await _paramsClient.Write(TelemetryParams.VelocityMaxParamName, MavParamType.MavParamTypeReal32, 
                    Math.Clamp(valueUp.ParamValue + TelemetryParams.PageDelta, 100f, 3000f), cancel); 
                break;
            ...
            case VehicleAction.PageDown:
                ...
                await _paramsClient.Write(TelemetryParams.VelocityMaxParamName, MavParamType.MavParamTypeReal32, 
                    Math.Clamp(valueDown.ParamValue - TelemetryParams.PageDelta, 100f, 3000f), cancel);
                break;
        ...
        _vehicleRenderer.WriteJournal(journal);
        _logRenderer.Add(journal);
    }
