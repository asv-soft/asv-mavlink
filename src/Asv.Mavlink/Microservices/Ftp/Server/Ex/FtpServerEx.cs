...
public partial class FtpServerEx : IFtpServerEx
{
    ...
    public async Task<ReadResult> FileRead(ReadRequest request, Memory<byte> buffer, CancellationToken cancel = default)
    {
        ...
        {
            _logger.ZLogError($"Unable to ReadFile. Requested skip offset more than session stream length #{request.Session}");
