...
public interface ICommandClient: IMavlinkMicroserviceClient
{
    ...
    Task<CommandAckPayload> CommandLong(
    ...
    /// <example>
    /// <code>
    /// Task&lt;SomePacketType&gt; commandResponse =
    ///     CommandLongAndWaitPacket&lt;SomePacketType&gt;(
    ///         MavCmd.SomeCommand,
    ///         1.0f, 2.0f, 3.0f, 4.0f, 5.0f, 6.0f, 7.0f);
    /// </code>
    /// </example>
    Task<TAnswerPacket> CommandLongAndWaitPacket<TAnswerPacket>(
        MavCmd command,
        float param1, 
        float param2, 
        float param3, 
    ...
    /// <param name="current">Not used (set false).</param>
    ...
    /// <returns>A task that represents the asynchronous operation.
    /// The task result contains the acknowledgment payload received from the vehicle.</returns>
    Task<CommandAckPayload> CommandInt(
        MavCmd command, 
        MavFrame frame, 
        bool current, 
        bool autoContinue, 
