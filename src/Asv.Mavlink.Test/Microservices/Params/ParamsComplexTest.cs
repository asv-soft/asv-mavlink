...
public class ParamsComplexTest : ComplexTestBase<ParamsClientEx, ParamsServerEx>
{
    ...
    public async Task WriteOnce_ServerShouldSetParameter_Success()
    {
        ...
        Assert.Equal(
            paramValue, 
            ParamsMeta.Encoding.ConvertFromMavlinkUnion(res.Payload.ParamValue, res.Payload.ParamType));
    ...
    public async Task ReadAll_ClientShouldReadAllParamsFromServer_Success()
    {
        ...
        foreach (var payload in receivedParams)
        {
            ...
            Assert.Equal(ParamsMeta.Encoding.ConvertToMavlinkUnion(item.Value.CurrentValue), payload.ParamValue);
        }
        ...
        IMavlinkContext core)
    {
        ...
        return new ParamsClientEx(client, _clientExConfig, ParamsMeta.Encoding, ParamsMeta.ClientParamsDescriptions);
    }
